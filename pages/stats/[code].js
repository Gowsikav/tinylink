...
const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/links/${params.code}`);
  ...
  if (!res.ok) {
    console.log("Failed to fetch link:", res.status);
...
export default function StatsPage({ link }) {
  return (
    <div style={{ maxWidth: 700, margin: "40px auto", padding: "0 16px" }}>
      ...
      <p><strong>Last clicked:</strong> {link.lastClicked ? format(new Date(link.lastClicked), "yyyy-MM-dd HH:mm") : "-"}</p>
      <p><strong>Created:</strong> {format(new Date(link.createdAt), "yyyy-MM-dd HH:mm")}</p>
    </div>
  );
}
